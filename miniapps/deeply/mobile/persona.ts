...
- \`sections\` 必填,${input.sections > 0
    ...
    : `**用户没指定节数,请你根据调研到的材料量自己决定**:合理范围 5-15 节,材料丰富 / 多个争议视角的话可以多分几节,材料较少 / 主题集中的话 5-8 节就行。不要少于 3 节也不要超过 30 节`}。每节:
  - \`title\` 8-30 中文字
...
${input.sections > 0
    ...
    : `用户没指定节数 —— 由你根据调研材料量自己决定。一般 5-15 节,材料丰富时多分,主题集中时少分。`}
...
- \`sections\`:${input.sections > 0
    ? `${input.sections} ± 20% 节`
    : `**用户没指定节数,请你根据资料量自定**:合理范围 5-15 节,资料长 / 内容密集时多分几节,短资料 / 单一主题 5-8 节就行。不要少于 3 节也不要超过 30 节`}。每节必须有 2-4 条 \`sources\`:
...
- \`sections\` ${input.sections > 0
    ? `以用户请求的 ${input.sections} 节为目标,允许 ±20% 浮动(下限 ${Math.max(2, Math.floor(input.sections * 0.8))} 节,上限 ${Math.ceil(input.sections * 1.2) + 1} 节)`
    : `**用户没指定节数,你根据这本书的章节结构和你能拿到的解读资料量自己决定**:合理范围 5-15 节,内容厚重 / 章节多的书往多分,小书 5-8 节就行。不要少于 3 节也不要超过 30 节`}。
