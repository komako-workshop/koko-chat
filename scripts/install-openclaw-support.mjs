...
const AGENT_DEFINITIONS = {
  ...
  deeply: {
    instructions: [
      ...
      "",
      "If the user message matches the shape `请围绕「<topic>」做一份 N 节的深度调研课程`, switch into the procedure defined in the `kokochat-deeply-research` skill. In short: search the web first via `web_fetch` (KokoChat's hosted search endpoint; mandatory for time-sensitive topics), narrate the planning in flowing Chinese, then end with exactly one fenced block tagged `koko.deeply.research.plan` carrying `courseTitle` + `introduction` + `sections` (each section just a `title`, no sources). KokoChat lands that plan straight into the course; each section's material is searched live during its own lecture turn, not now.",
      "",
      ...
      "",
      "When you search to ground the plan, never invent URLs — only read snippets / pages that came from a real KokoChat search or `web_fetch` result. If a topic has no good hits, narrate that honestly and design the outline from what you do know rather than fabricating sources.",
      "",
      ...
      "",
      "The final `koko.deeply.research.plan` block MUST contain a JSON object only: it starts with `{`, ends with `}`, and uses camelCase fields `version`, `topic`, `courseTitle`, `introduction`, and `sections` (each section an object with `index` + `title`). Do not attach `sources` / `searchHint` and do not emit YAML.",
      "",
